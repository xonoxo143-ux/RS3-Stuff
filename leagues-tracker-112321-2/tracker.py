...
def assistant_context(a, recommendations):
    ...
    lines = [
        ...
        f"- Access: **{account.get('access_mode', 'members')}**",
...
def main():
    ...
    f2p = state.get("access_mode") == "free_to_play" or state.get("manual_flags", {}).get("account_access") == "free_to_play"
    ...
    region_state = copy.deepcopy(state.get("regions", {}))
    unlocked = []
    ...
    # Automatic regions should follow live task count rather than requiring a manual edit.
    for entry in rules["regions"].get("automatic", []):
        if len(ids) >= int(entry["tasks"]) and entry["region"] not in unlocked:
            unlocked.append(entry["region"])
            auto = region_state.setdefault("automatic", [])
            if entry["region"] not in auto:
                auto.append(entry["region"])
    ...
    raw_relic_tier = relic_tier(rules, lp)
    current_relic_tier = min(raw_relic_tier, 3) if f2p else raw_relic_tier
    relics, relic_passives = resolve_relics(rules, state, current_relic_tier)

    # F2P stops after blessing tier 2 (3 blessing tasks). Clamp the effective
    # count so higher member-only blessing tiers never appear active.
    effective_blessing_count = min(blessing_count, 3) if f2p else blessing_count
    current_blessing_step, next_blessing = blessing_progress(
        rules, effective_blessing_count
    )
    if f2p and blessing_count >= 3:
        next_blessing = None
    blessings, blessing_passives, gods, dynamic = resolve_blessings(
        rules, state, effective_blessing_count
    )

    next_relic_data = next_relic(rules, lp)
    if f2p and current_relic_tier >= 3:
        next_relic_data = None

    next_region_data = next_region(rules, len(ids))
    if f2p and len(ids) >= 150:
        next_region_data = None

    progression = {
        "relic_tier": current_relic_tier,
        "next_relic": next_relic_data,
        "next_region": next_region_data,
        "current_blessing_step": current_blessing_step,
        "next_blessing": next_blessing,
        "access_mode": "free_to_play" if f2p else "members",
        "caps": (
            {"regions_total": 4, "relic_tier": 3, "blessing_tier": 2}
            if f2p
            else None
        ),
    }
    ...
    if f2p:
        validation_rules["regions"]["elective_thresholds"] = validation_rules[
            "regions"
        ].get("elective_thresholds", [])[:1]
